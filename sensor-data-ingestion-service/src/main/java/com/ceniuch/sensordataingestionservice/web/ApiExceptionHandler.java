 ...
 * ({@code application/problem+json}). Extends {@link ResponseEntityExceptionHandler}
 * so all standard Spring MVC failures (malformed JSON, missing headers, type
 * mismatches, bean validation) already return consistent 4xx problem responses;
 * only the domain-specific cases are added below.
 */
@Slf4j
