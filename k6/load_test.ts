// Ingestion load test with SLO thresholds.
//
// setup() registers a fixed pool of sensors + thresholds once, then a ramping
// scenario hammers the ingest hot path (which synchronously validates each key
// against the management service). Registration is deliberately kept out of the
// measured load so the numbers reflect steady-state ingestion, not onboarding.
//
//   k6 run load_test.ts
